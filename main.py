...
@commands.command(description="A method that will add arguments to word list and monitor messages.",brief="add_miner arg1 arg2")
async def add_word(ctx, *args):
    if len(args) < 1:
        ...
        await message_box("Succeeded", "You have successfully added them to the list, duplicate items are not included.", ctx)
"""
...
async def on_message(msg):
    ...
    member = msg.author
    if len(banned_item_list) == 0:
        message_box("Failed", "You need to add word to word list in order to remove spam messages and kick the sender out.", ctx)
    else:
